...
if ( $cgiparams{'ACTION'} eq $tr{'save'} ) {
	...
	unless ($errormessage) {
		...
		# To prevent loss of DHCP: Check for DHCP-WoL Mod and run instead of stock if it exists
		if (-e "${swroot}/mods/dhcpwol/usr/bin/smoothwall/writedhcp.pl") {
			system("${swroot}/mods/dhcpwol/usr/bin/smoothwall/writedhcp.pl");
		}
		else {
