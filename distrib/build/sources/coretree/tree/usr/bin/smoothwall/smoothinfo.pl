...
if ($smoothinfosettings{'SERVICES'} eq 'on') {
	...
	foreach my $key (sort keys %stockservices) {
		my $shortname = $stockservices{$key};
		my $status = &isrunning($shortname);
		$status = ucfirst ($status);
		print FILE "$key: $status\n";
		if ($key =~ /DHCP/) {
			...
			for (my $i=1; $i <= $#catleasesFILENAME; $i++){
				...
				if ($datLine =~ /^#/) {
					...
					if ($datLine =~ /^lease/) {
						...
						my @lineSplit = split(/ /,$datLine);		# Extract IP Address
						$dhcplIPAddy[$leaseCount] = $lineSplit[1];
					}
					elsif ($datLine =~ /^starts/) {
						my @lineSplit = split(/ /,$datLine);		# Extract Lease Start Date
						$dhcplStart[$leaseCount] = "$lineSplit[2] $lineSplit[3]";
					}
					elsif ($datLine =~ /^ends/) {
						my @lineSplit = split(/ /,$datLine);		# Extract Lease End Date
						$dhcplEnd[$leaseCount] = "$lineSplit[2] $lineSplit[3]";
					}
					elsif ($datLine =~ /^hardware ethernet/) {
						my @lineSplit = split(/ /,$datLine);		# Extract MAC Address
						...
						my @lineSplit = split(/ /,$datLine);		# Extract Host Name
			...
			for (my $i = $#dhcplIPAddy; $i >= 0; $i--) {
				...
				if ($dhcpprintvar =~ "True"){
					...
					if ($dhcplHostName[$i]) {
						...
						print FILE "IP: $dhcplIPAddy[$i] Lease started: $dhcplStart[$i] Ends: $dhcplEnd[$i] Mac: $dhcplMACAddy[$i] Host name: \n";
					}
				}
			}
			print FILE "No leases." if (!@dhcptemparray);
			print FILE "\[/code\]\[/info\]";
		}
	...
	if ($smoothinfosettings{'MODEXTRA'} eq 'on') {
		...
		while ( my $entry = readdir DIR ) {
			...
			if (-s "$modpath/$entry/smoothinfo.pl" && -x _) {
				my $title = ucfirst $entry;
				print FILE "\[info=\"$title\"\]\[quote\]";
